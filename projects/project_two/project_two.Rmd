...
```
# The plot and the summary make sense which is why the graph wasn't linear. The R squared being 0.4673, the predictive power of the weight determining hindfoot length is slightly below average so I wouldn't say it's great. Also the intercept being -32 tells us the expected value which makes no sense when its -32.  
```
...
# The assumptions were violated because they were not distributed which it was assumed that they were going to be distrubuted. 
...
# From looking at the facet plots, there does not look to be interspecific variation.
...
```
# The anova doesn't have the r sqaured or the residual standard error that the linear model does so it can't tell us about its predicitve power or how much deviation there is. 
```
...
```
# The original plot was difficult to read so the new plot looks better but the original predictor of weight predicting hindfoot length is still the same so that relationship still looks good but sex being a predictor as well changes the relationship slightly as the weight and sex were more closely related at least looking at their estimates. 
```
